...
export const Header: React.FC<HeaderProps> = ({ lang, setLang, cartCount, onCartClick }) => {
  ...
  const menuItems: Record<Language, string[]> = {
    TR: ['Yeni', 'Koleksiyonlar', 'Yuzuk', 'Kolye', 'Bilezik', 'Kupe', 'Pirlanta'],
  ...
  return (
    <header style={{
      ...
      <div style={{ maxWidth: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 32px' }}>
        ...
        >
          <span>PIVOT</span>
          <span style={{ color: '#B8962E' }}>GOLD</span>
          <span style={{ width: '6px', height: '6px', borderRadius: '50%', backgroundColor: '#B8962E', display: 'inline-block', marginLeft: '4px', marginBottom: '8px' }}></span>
        </Link>
        ...
        <div style={{
          ...
          <nav>
            {items.map((item, i) => (
              ...
              >
                {item}
                <span style={{ fontSize: '14px', color: '#B8962E' }}>{'→'}</span>
              </Link>
            ))}
          </nav>
          <div style={{ marginTop: '40px', paddingTop: '32px', borderTop: '1px solid #ECECEC' }}>
            ...
            <div style={{ display: 'flex', gap: '20px' }}>
              <a href="https://www.instagram.com/pivot_gold" target="_blank" rel="noopener noreferrer"
                style={{ fontFamily: '"Jost", sans-serif', fontSize: '13px', color: '#111111', textDecoration: 'none', fontWeight: 500 }}>Instagram</a>
              <a href="https://wa.me/905526487383" target="_blank" rel="noopener noreferrer"
                style={{ fontFamily: '"Jost", sans-serif', fontSize: '13px', color: '#111111', textDecoration: 'none', fontWeight: 500 }}>WhatsApp</a>
              <a href="https://pivotgold.etsy.com" target="_blank" rel="noopener noreferrer"
                style={{ fontFamily: '"Jost", sans-serif', fontSize: '13px', color: '#111111', textDecoration: 'none', fontWeight: 500 }}>Etsy</a>
            </div>
          </div>
        </div>
      )}
