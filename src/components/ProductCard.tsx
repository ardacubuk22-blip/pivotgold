    ...
    >
      <div style={{ position: 'relative', overflow: 'hidden', backgroundColor: '#F5F3EE', aspectRatio: '3/4' }}>
        <img
          src={product.image}
          alt={name}
          style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center', display: 'block', position: 'absolute', top: 0, left: 0, opacity: hovered ? 0.85 : 1, transition: 'opacity 0.4s ease' }}
        />
        <button
          onClick={handleWishlist}
          style={{ position: 'absolute', top: '10px', right: '10px', background: 'none', border: 'none', cursor: 'pointer', padding: '4px', opacity: hovered || wishlisted ? 1 : 0, transition: 'opacity 0.2s', zIndex: 2 }}
        >
          {React.createElement(Heart, { size: 18, strokeWidth: 1.5, color: wishlisted ? '#B8962E' : '#111111', fill: wishlisted ? '#B8962E' : 'none' })}
        </button>
      </div>
